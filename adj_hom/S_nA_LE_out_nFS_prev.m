    ...
    while inside && alive
        ...
        if E_group==1
            if rand<p12         % down scat
                ...
                r_i=ceil(r_n/(R/n_r));
                sum_c=sum_c+W*Chi1(r_i);       %previous adjoint
                sum_c2=sum_c2+(W*Chi1(r_i))^2;   %variation absolute error squared?
                alive=false;
                break
            end
        ...
        elseif E_group==2
            if r_n>=det1 && r_n<=det2   % detection
                ...
                W=W*(1-p2a);
                r_i=ceil(r_n/(R/n_r));
                sum_c=sum_c+W*Chi2(r_i);       %previous adjoint
                sum_c2=sum_c2+(W*Chi2(r_i))^2;   %variation absolute error squared?
                alive=false;
            end
        end
